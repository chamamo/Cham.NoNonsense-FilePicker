...
{
    public class AsyncDropBoxPickerTaskLoader : AbstractAsyncTaskLoader<DropboxApi.Entry>
    {
        protected DropboxApi DbApi;
        
        protected FilePickerMode Mode;

        public AsyncDropBoxPickerTaskLoader(Activity activity, Func<DropboxApi.Entry, bool> isItemVisible, DropboxApi dbApi, DropboxApi.Entry root, DropboxApi.Entry currentPath, FilePickerMode mode)
        ...
        protected override IEnumerable<DropboxApi.Entry> Load()
        {
            ...
            try
            {
                ...
                foreach (DropboxApi.Entry entry in dirEntry.Contents)
                {
                    if ((Mode == FilePickerMode.File || Mode == FilePickerMode.Dir) ||
                        entry.IsDir)
                    {
                        files.Add(entry);
                    }
